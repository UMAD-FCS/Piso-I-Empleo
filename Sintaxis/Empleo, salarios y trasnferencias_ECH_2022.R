...
varsimplant <- select(base, ID, ht11, HT19, ht13, e30, pobre)
...
bases_men <- bases_men %>% dplyr::mutate(bc_ipc = case_when(   
  mes == 1 ~  0.30530437,
  mes == 2 ~  0.30024958,
  mes == 3 ~  0.296416607,
  mes == 4 ~  0.293521663,
...
bases_men <- bases_men %>% dplyr::mutate(ipc_ene2022 = case_when(
  mes == 1 ~  1,
  mes == 2 ~  1.017847463,
  mes == 3 ~  1.032817647,
  mes == 4 ~  1.044243359,
...
bases_men <- bases_men %>% dplyr::mutate(pobre_06 = case_when(pobre == 1 ~ 1,
                                                              pobre == 0 ~ 2))

base <- base %>% dplyr::mutate(pobre_06 = case_when(pobre == 1 ~ 1,
                                                    pobre == 0 ~ 2))
...
rm(base_h, bases_men_h, base_unipersonales, base_unipersonales, base_tus, base_afam, 
...
Empleo_2022 <- rbind(t_01, t_02, t_03, t_04, t_05, t_06, t_07, t_08, t_09, t_10, t_11, t_12, t_13, t_14, t_15, t_16, t_17, t_18, t_19, t_20, t_21, t_22, t_23, t_24, t_25)
rio::export(Empleo_2022, "Empleo_2022.xlsx")


## Piso I

Empleo_2022 <- rio::import("Empleo_2022.xlsx")
Empleo_2022 <- Empleo_2022 %>% mutate(pobr = ifelse(is.na(SITUACION_HOGAR) | SITUACION_HOGAR =="", 0, 1),
                                      edad = ifelse(is.na(EDAD) | EDAD == "", 0, 1),
                                      sjef = ifelse(is.na(SEXO_JEFE) | SEXO_JEFE  ==  "", 0, 1))



piso_1  <- rio::import("C:/Users/Usuario/Dropbox/1. Unidad de Métodos y Acceso a Datos/1. Observatorio UMAD/GIT-HUB UMAD/Piso-I-Empleo/Data/Base_motor_empleo_17032023.xlsx")





Empleo_2022 <-  Empleo_2022  %>% mutate(AUX = case_when(NOMINDICADOR == "TASA DE ACTIVIDAD" & EDAD == "14 a 18" ~ "TASA DE ACTIVIDAD DE PERSONAS DE 14 A 18 AÑOS",
...
Empleo_2022 <- Empleo_2022  %>%  rename(NOMINDICADOR = AUX)

Empleo_2022 <- subset(Empleo_2022, select=c(CODIND,	NOMINDICADOR,	SEXO,	ASCENDENCIA,	QUINTIL, DEPARTAMENTO, URBANORURALUY,	PAIS,	FECHA,	FECHA2,	VALOR,	RESPONSABLE,	JERARQUIA,	SECTORPRODUCTIVO,	SECTORINSTITUCIONAL,	ACTIVIDADESECONOMICAS,	DIVISIÓNECONOMICA,	CONCEPTO,	CATEGORÍOCUP,	RELACIONFAMILIAR,	EDAD, SITUACION_HOGAR, RAMA_DE_ACTIVIDAD, TIPO_DE_OCUPACION, RESTRICCIONES_AL_EMPLEO, SEXO_JEFE, NOTA_INDICADOR, pobr, edad, sjef))



Empleo_2022 <-  Empleo_2022  %>% rename("QUINTIL DE INGRESO" = QUINTIL,
...
piso_1 <- rbind(piso_1, Empleo_2022)


piso_1[is.na(piso_1)]<-''
rio::export(piso_1, "C:/Users/Usuario/Dropbox/1. Unidad de Métodos y Acceso a Datos/1. Observatorio UMAD/GIT-HUB UMAD/Piso-I-Empleo/Data/Base_motor_empleo_15062023.xlsx")
rio::export(piso_1, "Base_motor_empleo_15062023.xlsx")
...
## Corrección de base motor por error de sintaxis en merge base panel

baseweb_mar_2023   <- rio::import("C:/Users/Usuario/Dropbox/1. Unidad de Métodos y Acceso a Datos/1. Observatorio UMAD/PISO II_MOTORES/2021/ECH 2021/Base Motor_Empleo-Salarios-Transferencias_19112022 (1).xlsx")

baseweb_mar_2023 <- baseweb_mar_2023 %>% filter(FECHA != 2021)
...
names <-names(baseweb_mar_2023)
names (Empleo_2021) = names


urbano  <- Empleo_2021 %>% filter(CORTE == "TOTAL" & REGIÓN_original == "URBANO (MÁS DE 5.000 HABITANTES)")
urbano <- urbano  %>% mutate(CORTE = "REGIÓN",
                             REGIÓN = "URBANO (MÁS DE 5.000 HABITANTES)",
                             JERARQUÍA = 0,
                             URB_TOT = "T")
...
urbano  <- urbano %>% filter(CODIND != 324)

Empleo_2021 <- rbind(Empleo_2021, urbano)

Empleo_2021 <- Empleo_2021  %>% mutate(elimino = case_when((CODIND == 317 | CODIND == 318 | CODIND == 319 | CODIND ==320) & CORTE == "REGIÓN" ~ 1, 
                                                           TRUE ~ 0))
Empleo_2021  <- Empleo_2021 %>% filter(elimino == 0)
Empleo_2021$elimino <- NULL


#baseweb_2021_mar2023 <- rbind(baseweb_mar_2023, Empleo_2021)
rio::export(Empleo_2021, "Exportaciones/revisión_20032023.xlsx")
