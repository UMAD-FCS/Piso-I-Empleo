...
* INDICADOR: Porcentaje de mayores de 65 años que no perciben jubilación ni pensión
* CÓDIGO:    332
...
**MODIFICACIÓN MANUAL: CÓDIGOS Y LOCAL J PARA VARIABLES AUXILIARES**
    ...
    *TOTAL PAÍS
	local i      = 1
	local codind = 333                  // Código de indicador
	...
	g transferencias = bc_pg912 + bc_pg922 + bc_pg911 + bc_pg921	
	g transferencia = 0
	replace transferencia = 1 if transferencias==0
	
	
	mean transferencia [aw=bc_pesoan] if bc_pe3>=65 
	matrix MATR  [`i',1]=  e(b)
	matrix MATR  [`i',2]=  0
	...
	mean transferencia [aw=bc_pesoan] if bc_pe3>=65 & region_3==`val'
	matrix MATR  [`i',1]=  e(b)
	matrix MATR  [`i',2]=  `j'
	matrix MATR  [`i',3]=  `anio'
    ...
    *
	...
	mean transferencia [aw=bc_pesoan] if bc_pe3>=65 & bc_pe2==`val'
	matrix MATR  [`i',1]=  e(b)
	matrix MATR  [`i',2]=  `j'
    matrix MATR  [`i',3]=  `anio'
    ...
    *
	...
	local j  = 	5
	foreach val of numlist 1/2  {
	mean transferencia [aw=bc_pesoan] if bc_pe3>=65 & bd_e29_1==`val'
	matrix MATR  [`i',1]=  e(b)
	matrix MATR  [`i',2]=  `j'
    matrix MATR  [`i',3]=  `anio'
    ...
    *
	...
	mean transferencia [aw=bc_pesoan] if bc_pe3>=65 & bd_quintilesy==`val'
	matrix MATR  [`i',1]=  e(b)
	matrix MATR  [`i',2]=  `j'
    matrix MATR  [`i',3]=  `anio'
    ...
    *
	...
	mean transferencia [aw=bc_pesoan] if bc_pe3>=65 & pobre06==`val'
	matrix MATR  [`i',1]=  e(b)
	matrix MATR  [`i',2]=  `j'
    matrix MATR  [`i',3]=  `anio'
...
g CODIND                 = 333
g NOMINDICADOR           = "PORCENTAJE DE MAYORES DE 65 AÑOS QUE NO PERCIBEN JUBILACIONES NI PENSIONES"
g SEXO                   = "NA"
g ASCENDENCIA    		 = "NA"
g QUINTIL       		 = "NA"
g DEPARTAMENTOUY		 = "NA"
...
replace NOMINDICADOR    = "PORCENTAJE DE MAYORES DE 65 AÑOS QUE NO PERCIBEN JUBILACIONES NI PENSIONES EN HOGARES NO EN SITUACIÓN DE POBREZA"    if AUXILIAR==12
replace NOMINDICADOR    = "PORCENTAJE DE MAYORES DE 65 AÑOS QUE NO PERCIBEN JUBILACIONES NI PENSIONES EN HOGARES EN SITUACIÓN DE POBREZA"       if AUXILIAR==13
...
************************************************************************************************
    ...
    *PAÍS URBANO
	...
	g pension = 0
	replace pension = 1 if pensiones>0
		
	mean pension [aw=bc_pesoan] if bc_pe3>=65 & bc_filtloc==1
	matrix MATR  [`i',1]=  e(b)
	matrix MATR  [`i',2]=  0
	matrix MATR  [`i',3]=  `anio'
	...
	local j  = 	1
	foreach val of numlist 1/2  { 
	mean pension [aw=bc_pesoan] if bc_pe3>=65 & bc_pe2==`val' & bc_filtloc==1
	matrix MATR  [`i',1]=  e(b)
	matrix MATR  [`i',2]=  `j'
    matrix MATR  [`i',3]=  `anio'
    ...
    *
	...
	local j  = 	3
	foreach val of numlist 1/5  {
	mean pension [aw=bc_pesoan] if bc_pe3>=65 & bd_quintilesy==`val' & bc_filtloc==1
	matrix MATR  [`i',1]=  e(b)
	matrix MATR  [`i',2]=  `j'
    matrix MATR  [`i',3]=  `anio'
