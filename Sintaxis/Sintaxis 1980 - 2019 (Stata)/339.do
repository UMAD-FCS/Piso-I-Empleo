...
* INDICADOR: Porcentaje de hogares que perciben asignaciones familiares
* CÓDIGO:    338
...
**MODIFICACIÓN MANUAL: CÓDIGOS Y LOCAL J PARA VARIABLES AUXILIARES**
    ...
    *TOTAL PAÍS
	local i      = 1
	local codind = 339                  // Código de indicador
	...
	egen h_asignacion = sum (bc_afam), by (bc_correlat)
	replace h_asignacion=1 if h_asignacion>0
    ...
    mean h_asignacion [aw=bc_pesoan] if bc_pe4==1
	matrix MATR  [`i',1]=  e(b)
	matrix MATR  [`i',2]=  0
	...
	mean h_asignacion [aw=bc_pesoan] if bc_pe4==1 & region_3==`val'
	matrix MATR  [`i',1]=  e(b)
	matrix MATR  [`i',2]=  `j'
	matrix MATR  [`i',3]=  `anio'
    ...
    *
	...
	mean h_asignacion [aw=bc_pesoan] if bc_pe4==1 & sexojefe==`val'
	matrix MATR  [`i',1]=  e(b)
	matrix MATR  [`i',2]=  `j'
    matrix MATR  [`i',3]=  `anio'
    ...
    *
	...
	mean h_asignacion [aw=bc_pesoan] if bc_pe4==1 & bd_quintilesy==`val'
	matrix MATR  [`i',1]=  e(b)
	matrix MATR  [`i',2]=  `j'
    matrix MATR  [`i',3]=  `anio'
    ...
    *
	...
	mean h_asignacion [aw=bc_pesoan] if bc_pe4==1 & pobre06==`val'
	matrix MATR  [`i',1]=  e(b)
	matrix MATR  [`i',2]=  `j'
    matrix MATR  [`i',3]=  `anio'
...
g CODIND                 = 339
g NOMINDICADOR           = "PORCENTAJE DE HOGARES QUE PERCIBEN ASIGNACIONES FAMILIARES"
g SEXO                   = "NA"
g ASCENDENCIA    		 = "NA"
g QUINTIL       		 = "NA"
g DEPARTAMENTOUY		 = "NA"
...
replace NOMINDICADOR    = "PORCENTAJE DE HOGARES CON JEFATURA MASCULINA QUE PERCIBEN ASIGNACIONES FAMILIARES" if AUXILIAR==4
replace NOMINDICADOR    = "PORCENTAJE DE HOGARES CON JEFATURA FEMENINA QUE PERCIBEN ASIGNACIONES FAMILIARES"  if AUXILIAR==5
...
replace NOMINDICADOR    = "PORCENTAJE DE HOGARES NO EN SITUACIÓN DE POBREZA QUE PERCIBEN ASIGNACIONES FAMILIARES"    if AUXILIAR==11
replace NOMINDICADOR    = "PORCENTAJE DE HOGARES EN SITUACIÓN DE POBREZA QUE PERCIBEN ASIGNACIONES FAMILIARES"       if AUXILIAR==12
