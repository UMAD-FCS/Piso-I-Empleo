...
base            <- rio::import("Bases/ECH_implantacion_2024.Rdata")
bases_men       <- rio::import("Bases/ECH_panel_2024.Rdata")
...
bases_men <- bases_men %>% dplyr::mutate(bc_register = ifelse(f82 == 2, 1, 0))
base <- base %>% dplyr::mutate(bc_register = ifelse(f82 == 2, 1, 0))
...
base <- base %>% dplyr::mutate(tus = ifelse(e584==1, 1, 0)) # Cambió la pregunta
...
Empleo_2024 <- rbind(t_01, t_02, t_03, t_04, t_05, t_06, t_07, t_08, t_09, t_10, t_11, t_12, t_13, t_14, t_15, t_16, t_17, t_18, t_19, t_20, t_21, t_22, t_23, t_24, t_25)
Empleo_2024$VALOR <- as.numeric(Empleo_2024$VALOR)
rio::export(Empleo_2024, "Export actualiza 2024/Empleo_2024.xlsx")
...
Empleo_2024 <- rio::import("Export actualiza 2024/Empleo_2024.xlsx")
Empleo_2024 <- Empleo_2024 %>% mutate(pobr = ifelse(is.na(SITUACION_HOGAR) | SITUACION_HOGAR =="", 0, 1),
                                      edad = ifelse(is.na(EDAD) | EDAD == "", 0, 1),
                                      sjef = ifelse(is.na(SEXO_JEFE) | SEXO_JEFE  ==  "", 0, 1))



piso_1  <- rio::import("C:/Users/Usuario/Dropbox/1. Unidad de Métodos y Acceso a Datos/1. Observatorio UMAD/GIT-HUB UMAD/Piso-I-Empleo/Data/Base_motor_empleo_03052024.xlsx")

piso_1 <- piso_1 %>%
  mutate(VALOR = if_else(
    NOMINDICADOR == "PORCENTAJE DE OCUPADOS NO REGISTRADOS EN SEGURIDAD SOCIAL EN OCUPACIÓN PRINCIPAL",
    round((1 - as.numeric(VALOR)), 3),
    as.numeric(VALOR)
  ))


Empleo_2024 <-  Empleo_2024  %>% mutate(AUX = case_when(NOMINDICADOR == "TASA DE ACTIVIDAD" & EDAD == "14 a 18" ~ "TASA DE ACTIVIDAD DE PERSONAS DE 14 A 18 AÑOS",
                                                        ...
                                                        TRUE ~ NOMINDICADOR))
Empleo_2024 <- select(Empleo_2024, -NOMINDICADOR)
Empleo_2024 <- Empleo_2024  %>%  rename(NOMINDICADOR = AUX)

Empleo_2024 <- subset(Empleo_2024, select=c(CODIND,	NOMINDICADOR,	SEXO,	ASCENDENCIA,	QUINTIL, DEPARTAMENTO, URBANORURALUY,	PAIS,	FECHA,	FECHA2,	VALOR,	RESPONSABLE,	JERARQUIA,	SECTORPRODUCTIVO,	SECTORINSTITUCIONAL,	ACTIVIDADESECONOMICAS,	DIVISIÓNECONOMICA,	CONCEPTO,	CATEGORÍOCUP,	RELACIONFAMILIAR,	EDAD, SITUACION_HOGAR, RAMA_DE_ACTIVIDAD, TIPO_DE_OCUPACION, RESTRICCIONES_AL_EMPLEO, SEXO_JEFE, NOTA_INDICADOR, pobr, edad, sjef))



Empleo_2024 <-  Empleo_2024  %>% rename("QUINTIL DE INGRESO" = QUINTIL,
...
names <- names(piso_1)
names (Empleo_2024) = names

piso_1 <- rbind(piso_1, Empleo_2024)


piso_1[is.na(piso_1)]<-''
rio::export(piso_1, "C:/Users/Usuario/Dropbox/1. Unidad de Métodos y Acceso a Datos/1. Observatorio UMAD/GIT-HUB UMAD/Piso-I-Empleo/Data/Base_motor_empleo_13052025.xlsx")
rio::export(piso_1, "Export actualiza 2024/Base_motor_empleo_13052025.xlsx")
...
Empleo_2024 <- rio::import("Export actualiza 2024/Empleo_2024.xlsx")
baseweb     <- rio::import("Versión pervia web/Base Motor_Empleo-Salarios-Transferencias_13.05.2024_web.xlsx") # Base descargada de drive compartido con economía
...
baseweb_2024 <- rbind(baseweb, Empleo_2023)
...
baseweb_2024 <- rbind(baseweb_2024, urbano)


# Se eliminan algunas filas que no se suben a la web


baseweb_2024 <- baseweb_2024 %>% mutate(filtro = ifelse(FECHA == 2023 & CORTE == "REGIÓN" & (CODIND == 317 | 
                                                                                               CODIND == 319 | 
                                                                                               CODIND == 320 | 
                                                                                               CODIND == 318), 1, 0))

baseweb_2024 <- baseweb_2024 %>%  subset(filtro == 0)
baseweb_2024 <- select(baseweb_2024, -filtro)
...
rio::export(baseweb_2024, "Export actualiza 2024/Empleo_2023_piso II_17052024.xlsx")
